...
local catan_placeholder_name = "catan:placeholder"

minetest.register_node(catan_placeholder_name, {
	drawtype = "airlike",
	paramtype = "light",
	...
	while pos.x <= pos2.x do
		...
		while pos.y <= pos2.y do
			...
			while pos.z <= pos2.z do
				...
				if node.name ~= catan_placeholder_name then
					...
					node.name = catan_placeholder_name --set node name
...
worldedit.suppress = function(pos1, pos2, nodename)
	...
	if nodename == catan_placeholder_name then
		return 0
	end
		...
		while pos.y <= pos2.y do
			...
			while pos.z <= pos2.z do
				...
				if node.name == nodename then --node found
					count = count + 1
				elseif node.name ~= catan_placeholder_name then --hide other nodes
					local data = get_meta(pos):to_table() --obtain metadata of original node
					data.fields.worldedit_placeholder = node.name --add the node's name
					node.name = catan_placeholder_name --set node name
					add_node(pos, node) --add placeholder node
...
worldedit.restore = function(pos1, pos2)
	...
	local pos1, pos2 = worldedit.sort_pos(pos1, pos2)
	local nodes = minetest.find_nodes_in_area(pos1, pos2, catan_placeholder_name)
	local get_node, get_meta, add_node = minetest.get_node, minetest.get_meta, minetest.add_node
	for _, pos in ipairs(nodes) do
		local node = get_node(pos)
